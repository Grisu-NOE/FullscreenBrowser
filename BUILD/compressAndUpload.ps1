...
Write-Host "Try Start-Process"
$process = "$Env:BUILD_SOURCESDIRECTORY\BUILD\7z\7z.exe"
Start-Process -FilePath $process -ArgumentList "a -t7z -m0=lzma2 -mx=$Env:CompressionLevel -mfb=64 -md=128m -ms=on ""$Env:BUILD_STAGINGDIRECTORY\$fileVersion.7z"" ""$binaries""" -Wait -NoNewWindow -PassThru
